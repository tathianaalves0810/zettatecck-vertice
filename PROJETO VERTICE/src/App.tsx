...
if (typeof document !== 'undefined') {
  ...
  s.textContent = `
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    body,html{width:100%;min-height:100vh;overflow-x:hidden;font-family:'Inter',system-ui,-apple-system,sans-serif;background:#EEF2F7;}
    #root{min-height:100vh;}
    input,select,textarea,button{font-family:'Inter',system-ui,-apple-system,sans-serif;}
    table{border-collapse:collapse;}
    td,th{font-family:'Inter',system-ui,-apple-system,sans-serif;}
    #pedidos-scroll{scrollbar-gutter:stable;}
    #pedidos-scroll::-webkit-scrollbar{height:8px;width:8px;}
    #pedidos-scroll::-webkit-scrollbar-track{background:#EEF2F7;border-radius:4px;}
    #pedidos-scroll::-webkit-scrollbar-thumb{background:#4A7FA0;border-radius:4px;}
    #pedidos-scroll::-webkit-scrollbar-thumb:hover{background:#1E3A5F;}
    #pedidos-scroll::-webkit-scrollbar-corner{background:#EEF2F7;}
    .modal-body::-webkit-scrollbar{width:6px;}
    .modal-body::-webkit-scrollbar-track{background:#F8FAFC;border-radius:4px;}
    .modal-body::-webkit-scrollbar-thumb{background:#4A7FA0;border-radius:4px;}
    .modal-body::-webkit-scrollbar-thumb:hover{background:#1E3A5F;}
    .tbl-scroll-sticky{overflow-x:scroll;position:sticky;bottom:0;z-index:10;background:#fff;border-top:1px solid #E2E8F0;}
    .tab-btn{transition:all 0.15s ease;}
    .tab-btn:hover{background:rgba(255,255,255,.12)!important;color:rgba(255,255,255,.9)!important;}
    tr.tbl-row:hover td{background:#F0F6FB!important;}
...
const SC = {
  Concluído: { bg: '#ECFDF5', text: '#059669', dot: '#10B981' },
  Aguardando: { bg: '#FFFBEB', text: '#D97706', dot: '#F59E0B' },
  'Faturado Inicial': { bg: '#EFF6FF', text: '#2563EB', dot: '#3B82F6' },
  'Entregue Parcial': { bg: '#FFF7ED', text: '#C2410C', dot: '#F97316' },
  'ENTREGUE TOTAL': { bg: '#ECFDF5', text: '#059669', dot: '#10B981' },
  'ENTREGUE PARCIAL': { bg: '#FFF7ED', text: '#C2410C', dot: '#F97316' },
  'NÃO ENTREGUE': { bg: '#FFF1F2', text: '#E11D48', dot: '#F43F5E' },
  'Pago total': { bg: '#ECFDF5', text: '#059669', dot: '#10B981' },
  'Pago parcial': { bg: '#FFFBEB', text: '#D97706', dot: '#F59E0B' },
  'Não pago': { bg: '#FFF1F2', text: '#E11D48', dot: '#F43F5E' },
  ativo: { bg: '#ECFDF5', text: '#059669', dot: '#10B981' },
  pendente: { bg: '#FFFBEB', text: '#D97706', dot: '#F59E0B' },
  rejeitado: { bg: '#FFF1F2', text: '#E11D48', dot: '#F43F5E' },
};
...
function Badge({ s }) {
  const c = SC[s] || { bg: '#F1F5F9', text: '#64748B', dot: '#94A3B8' };
  return (
    <span
      style={{
        display: 'inline-flex',
...
// fs() converte px para rem já escalado para zoom 175%
// html font-size = 57.14% de 16px = 9.14px, então 1rem = 9.14px
// Para obter o tamanho visual desejado em px: valor / 9.14
const fs = (px) => `${(px / 9.142).toFixed(3)}rem`;
const INP = {
  width: '100%',
  padding: '10px 14px',
...
const SAVE_BTN = {
  ...
  fontSize: 13,
  letterSpacing: 0.3,
  boxShadow: '0 2px 8px rgba(30,58,95,.3)',
};
const CANCEL_BTN = {
  padding: '10px 20px',
  border: '1.5px solid #E8EDF4',
...
function Modal({ title, onClose, children, wide, zoom }) {
  return (
    <div
      style={{
        position: 'fixed',
      ...
      >
        <div
          style={{
            ...
            alignItems: 'center',
            flexShrink: 0,
            background: 'linear-gradient(135deg,#0A0F1E,#0F3460)',
...
function KPI({ label, value, sub, color, icon, pct }) {
  return (
    <div
      style={{
        background: '#fff',
    ...
    >
      <div
        style={{
          flex: '0 0 45%',
          background:
            'linear-gradient(160deg,#0F1B26 0%,#1A2A3A 60%,#0A0F1E 100%)',
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          justifyContent: 'center',
      ...
      >
        <div
          style={{
            ...
            height: 420,
            borderRadius: '50%',
            border: '1px solid rgba(126,196,224,.08)',
            top: -80,
            left: -120,
          }}
        />
        <div
          style={{
            ...
            height: 280,
            borderRadius: '50%',
            border: '1px solid rgba(192,112,96,.06)',
            bottom: 40,
            right: -80,
          }}
        />
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          {/* VÉRTICE logo em destaque */}
          <div
            style={{
              display: 'flex',
              alignItems: 'center',
          ...
          >
            <svg width="80" height="80" viewBox="-55 -55 110 110">
              ...
              />
              <polygon
                points="0,-33 28,16 -28,16"
                fill="none"
                stroke="#4A7FA0"
                strokeWidth="1"
                opacity="0.4"
              />
              <circle cx="0" cy="-33" r="7" fill="#C07060" />
              <circle cx="28" cy="16" r="3" fill="#C07060" opacity="0.5" />
              <circle cx="-28" cy="16" r="3" fill="#C07060" opacity="0.5" />
              <line
                x1="0"
                y1="-33"
...
function UsuariosTab() {
  ...
  const [editandoPerfil, setEditandoPerfil] = useState({}); // id -> perfil editando
  ...
  async function aprovar(id) {
    const perfil = perfilPendente[id] || 'Compras';
  ...
  async function salvarPerfil(id) {
    const novoPerfil = editandoPerfil[id];
    if (!novoPerfil) return;
    await sbUpdate('usuarios', id, { perfil: novoPerfil });
    setEditandoPerfil((p) => ({ ...p, [id]: null }));
    carregar();
  }
            ...
            >
              ...
              >
                <div>
                  ...
                  <select
                    style={{ ...INP, width: 180, padding: '6px 10px' }}
                    value={perfilPendente[u.id] || 'Engenharia'}
                    onChange={(e) =>
                      setPerfilPendente((p) => ({
                        ...p,
                        [u.id]: e.target.value,
          ...
          <tbody>
            {ativos.map((u, i) => (
              <tr key={u.id} style={{ background: i % 2 ? '#FAFBFC' : '#fff' }}>
                ...
                <TD>
                  {editandoPerfil[u.id] !== undefined &&
                  editandoPerfil[u.id] !== null ? (
                    <div
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          setEditandoPerfil((p) => ({ ...p, [u.id]: null }))
                        }
                        style={{
                          padding: '4px 8px',
                          background: '#F1F5F9',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      >
        ...
        >
          ...
          <option value="ENTREGUE TOTAL">Entregue</option>
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: '#64748B', marginTop: 4 }}>
                💡 Nestlé, Garoto, CPW, Purina = 35% · Outros = 40% do valor do
                projeto
              </div>
            </div>
            <div style={{ gridColumn: 'span 2' }}>
              <label style={LBL}>
                {form.status === 'ENTREGUE TOTAL'
                  ? '📅 Data de Entrega'
...
function PedidosTab() {
  ...
  async function save() {
    ...
    // valida: itens faturados precisam de previsão de entrega
    const semPrevEntrega = itensPendentes.filter(
      (it) => it.faturado && !it.prevEntrega
    );
    if (semPrevEntrega.length > 0) {
      alert(
        `⚠️ Preencha a previsão de entrega para ${
          semPrevEntrega.length
        } item(s) faturado(s):\n${semPrevEntrega
          .map((it) => it.codigo)
          .join(', ')}`
      );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      >
        <div
          ...
          onMouseDown={(e) => {
            const el = document.getElementById('pedidos-scroll');
            el._drag = true;
          ...
          >
            <thead>
              <tr>
                ...
                <TH>Status</TH>
                <TH>NF-e</TH>
                <TH style={{ whiteSpace: 'nowrap' }}>Prazo Fatura</TH>
                <TH style={{ whiteSpace: 'nowrap' }}>Prazo Entrega</TH>
                <TH>Pendência</TH>
                <TH style={{ minWidth: 240 }}>Itens Pendentes</TH>
                <TH>Obs</TH>
              </tr>
            </thead>
            <tbody>
                  ...
                  >
                    ...
                    <TD style={{ fontSize: 11 }}>
                      <div
                        style={{
                          fontWeight: 600,
                          color: p.frete === 'FOB' ? '#C07060' : '#64748B',
                        }}
                      >
                        {p.frete || 'CIF'}
                      </div>
                      {p.valor_frete > 0 && (
                        <div style={{ fontSize: 10, color: '#C07060' }}>
                          +{fmt(p.valor_frete)}
                        </div>
                      )}
                    </TD>
                    ...
                    <TD
                      style={{
                        ...
                        color: (() => {
                          ...
                          if (itens.length === 0) return '#94A3B8';
                          if (itens.every((it) => it.faturado)) {
                            // verde só se todos têm prevEntrega já vencida (entregues de fato)
                            const hoje = new Date();
                            hoje.setHours(0, 0, 0, 0);
                            const todosEntreguesReal = itens.every(
                              (it) =>
                                it.prevEntrega &&
                                parseLocal(it.prevEntrega) <= hoje
                            );
                            return todosEntreguesReal ? '#10B981' : '#C07060'; // laranja = faturado mas aguardando entrega
                          }
                          return '#EF4444'; // ainda tem itens não faturados = vermelho
                        })(),
                      }}
                    >
                      {p.pendencia}
                    ...
                    <TD style={{ minWidth: 240, maxWidth: 340 }}>
                      {(() => {
                        ...
                        const algumPendente = itens.some((it) => !it.faturado);
                        if (itens.length === 0)
                          return (
                        ...
                        // todos faturados E entregues = badge verde
                        if (todosEntreguesReal) {
                          return (
                            <div
                              style={{
                                display: 'flex',
                              ...
                              {itens.map((it, idx) => (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    {it.prevEntrega
                                      ? ` · Entrega: ${fmtDate(it.prevEntrega)}`
                                      : ''}
                                  </span>
                                </div>
                              ))}
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <select
                style={INP}
                value={form.frete || 'CIF'}
                onChange={(e) => set('frete', e.target.value)}
              >
                {[
                  'CIF',
                  'FOB',
                  'Transporte Destinatário',
                  'Transporte Remetente',
                  'Sem Transporte',
                ].map((f) => (
            ...
            {form.frete === 'FOB' && (
              <div>
                <label style={{ ...LBL, color: '#C07060' }}>
                  💰 Valor do Frete FOB (R$)
                </label>
                <input
                  style={{ ...INP, borderColor: '#C07060' }}
                  type="number"
                  value={form.valor_frete || ''}
                  onChange={(e) => set('valor_frete', e.target.value)}
                  placeholder="Não soma no total do pedido"
                />
              </div>
            )}
            <div>
            ...
            {form.status === 'Faturado Inicial' && (
              ...
              >
                ...
                <div>
                  ...
                  >
                    Lembrete: confirme a nova data de entrega!
                  </div>
                  <div
                    style={{ fontSize: 11, color: '#B45309', lineHeight: 1.5 }}
                  >
                    O pedido foi faturado — a data de entrega original pode ter
                    mudado. Atualize o campo <strong>"Prazo de Entrega"</strong>{' '}
                    abaixo com a nova previsão considerando a data de
                    faturamento.
                  </div>
                </div>
              </div>
                      ...
                      {!item.faturado ? (
                        ...
                        <>
                          ...
                          >
                            ...
                            {/* Previsão de entrega do item faturado */}
                            <div
                              style={{
                                display: 'flex',
                                alignItems: 'center',
                            ...
                            >
                              ...
                              <input
                                type="date"
                                value={item.prevEntrega || ''}
                                onChange={(e) => {
                                  const arr = [...(form.itensPendentes || [])];
                                  arr[idx] = {
                                    ...arr[idx],
                                    prevEntrega: e.target.value,
                                  };
                                  set('itensPendentes', arr);
                                }}
                                style={{
                              ...
                              {item.prevEntrega &&
                                (() => {
                                  const hoje = new Date();
                                  hoje.setHours(0, 0, 0, 0);
                                  const prev = parseLocal(item.prevEntrega);
                                  if (prev < hoje)
                                    return (
                                      <span
                                        style={{
...
function FornecedoresTab() {
  ...
  if (loading) return <Spin />;
  const filt = fornecedores.filter((f) => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      >
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="🔍 Buscar empresa, responsável, e-mail..."
          style={{ ...INP, flex: 1, minWidth: 180 }}
        />
        {search && (
          <button
...
function Dashboard() {
  ...
  const { data: fretes, loading: lFr } = useCRUD(
    'fretes',
  ...
  const alertasHoje = useMemo(() => {
    ...
    pedidos.forEach((p) => {
      ...
      if (p.itens_pendentes) {
        ...
        itens.forEach((it) => {
          // alerta: item ainda não faturado e prazo vencido
          ...
          // NOVO: item faturado mas entrega prevista vencida
          if (it.faturado && it.prevEntrega) {
            const dt = parseLocal(it.prevEntrega);
            if (dt && dt < hj)
              alertas.push({
                tipo: 'item_entrega_atrasada',
                label: '🚛 Entrega item faturado',
                color: '#DC2626',
                bg: '#FEE2E2',
                pedido: p,
                detalhe: `${it.codigo} x${
                  it.quantidade
                } — Entrega prev: ${fmtDate(it.prevEntrega)}`,
              });
          }
        });
      }
  ...
  if (lP || lPed || lPag || lFr) return <Spin />;

  // fretes do dia de hoje
  const hojeStr = hoje.toISOString().split('T')[0];
  const fretesHoje = fretes
    .filter((f) => f.data_coleta === hojeStr)
    .sort((a, b) =>
      (a.horario_coleta || '99:99').localeCompare(b.horario_coleta || '99:99')
    );
  ...
  const getMotivo = (p) => {
    ...
    if (p.status === 'Faturado Inicial') return 'faturado_inicial';
  ...
  const pendencias = pedidos.filter((p) => {
    ...
    if (p.status === 'Faturado Inicial') return true;
  ...
  const counts = {
    ...
    nao_faturou: pendencias.filter((p) => getMotivo(p) === 'nao_faturou')
      .length,
    faturado_inicial: pendencias.filter(
      (p) => getMotivo(p) === 'faturado_inicial'
    ).length,
  ...
  const MC = {
    ...
    faturado_inicial: { label: '🔵 Faturado Inicial', color: '#2563EB' },
    entregue_parcial: { label: '🟠 Entregue Parcial', color: '#F97316' },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      >
        <KPI
          ...
          sub={`${
            projetos.filter((p) => p.status === 'ENTREGUE TOTAL').length
          } entregues`}
          color="#3B82F6"
        />
        <KPI
              ...
              <button
                onClick={() => {
                  ...
                  const corMap = {
                    ...
                    item_entrega_atrasada: '#DC2626',
                  };
                  const bgMap = {
                    fat_atrasado: '#FEE2E2',
                    ent_atrasado: '#FEE2E2',
                    item_atrasado: '#FFF7ED',
                    item_entrega_atrasada: '#FEE2E2',
                  };
                  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Alertas do Dia</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;font-size:10px;color:#1E293B;}.header{background:linear-gradient(135deg,#0A0F1E,#0F3460);color:#fff;padding:16px 28px;display:flex;align-items:center;justify-content:space-between;}.meta-bar{background:#C07060;color:#fff;padding:8px 28px;font-size:9px;font-weight:700;display:flex;justify-content:space-between;}.content{padding:20px 28px;}.alerta{display:flex;align-items:center;gap:12px;padding:10px 14px;border-radius:8px;margin-bottom:8px;font-size:10px;}.footer{background:#F8FAFC;border-top:2px solid #C07060;padding:10px 28px;display:flex;justify-content:space-between;font-size:8px;color:#64748B;margin-top:20px;}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style></head><body><div class="header"><div style="display:flex;align-items:center;gap:14px;"><div style="width:52px;height:52px;border-radius:12px;background:#8B3A2A;display:flex;align-items:center;justify-content:center;border:1.5px solid rgba(192,112,96,.5);"><svg width="48" height="48" viewBox="-55 -55 110 110"><polygon points="0,-48 42,24 -42,24" fill="none" stroke="#4A7FBF" stroke-width="2.5"/><polygon points="0,-33 28,16 -28,16" fill="none" stroke="#4A7FBF" stroke-width="1" opacity="0.4"/><circle cx="0" cy="-33" r="7" fill="#C07060"/><circle cx="28" cy="16" r="3" fill="#C07060" opacity="0.5"/><circle cx="-28" cy="16" r="3" fill="#C07060" opacity="0.5"/><line x1="0" y1="-33" x2="0" y2="16" stroke="#C07060" stroke-width="2" opacity="0.7"/><line x1="0" y1="-33" x2="28" y2="16" stroke="#C07060" stroke-width="1" opacity="0.3"/><line x1="0" y1="-33" x2="-28" y2="16" stroke="#C07060" stroke-width="1" opacity="0.3"/></svg></div><div><div style="font-size:22px;font-weight:800;letter-spacing:-0.5px;line-height:1;">VÉRTICE</div><div style="font-size:8px;color:rgba(255,255,255,.4);letter-spacing:1.5px;text-transform:uppercase;margin-top:2px;">gestão de suprimentos</div></div><div style="width:1px;height:32px;background:rgba(255,255,255,.12);margin:0 8px;"></div><div style="display:flex;align-items:center;gap:6px;opacity:0.5;"><img src="${LOGO}" style="width:18px;height:18px;border-radius:4px;object-fit:cover;" alt="Zettatecck"/><span style="color:rgba(255,255,255,.7);font-size:9px;">Zettatecck Projetos</span></div></div><div style="text-align:right;"><div style="color:#fff;font-size:13px;font-weight:700;">Alertas do Dia</div><div style="color:#94A3B8;font-size:9px;margin-top:3px;">${dataHoje}</div></div></div><div class="meta-bar"><span>Total de alertas: <strong>${
                    alertasHoje.length
                    ...
                    .join(
                      ''
                    )}</div><div class="footer"><span><strong>VÉRTICE</strong> — Gestão de Suprimentos · ZETTATECCK PROJETOS</span><span>${
                    alertasHoje.length
      ...
      {fretesHoje.length > 0 && (
        ...
        >
          ...
          >
            ...
            >
              {fretesHoje.length} frete{fretesHoje.length !== 1 ? 's' : ''} hoje
            </span>
          </div>
          <div
            style={{
          ...
          >
            {fretesHoje.map((f) => (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div
                    style={{ fontWeight: 800, fontSize: 13, color: '#0D1B2A' }}
                  >
                    {f.numero}{' '}
                    <span
                      style={{
                  ...
                  <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 2 }}>
                    Projeto{' '}
                    <strong style={{ color: '#3B82F6' }}>{f.projeto}</strong>
                    {f.cte ? ` · CT-e ${f.cte}` : ''}
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span
                    style={{
                      ...
                      borderRadius: 20,
                      background:
                        f.tipo === 'Remetente' ? '#EFF6FF' : '#FFF7ED',
                      color: f.tipo === 'Remetente' ? '#1E40AF' : '#C2410C',
                    }}
                  >
                    {f.tipo === 'Remetente'
                      ? '📤 Remetente'
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[
              ...
              ['faturado_inicial', `Fat. Inicial (${counts.faturado_inicial})`],
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              ...
              {filtrados.map((p, i) => {
                ...
                // monta itens pendentes
                let itens = [];
                try {
                  if (p.itens_pendentes) itens = JSON.parse(p.itens_pendentes);
                } catch {}
                const itensPend = itens.filter((it) => !it.faturado);
                const itensFat = itens.filter((it) => it.faturado);
                return (
                  ...
                  >
                    ...
                    <TD style={{ minWidth: 220 }}>
                      {itens.length === 0 ? (
                        ...
                        >
                          {itens.map((it, idx) => {
                            const bg = it.faturado ? '#7C3A2A' : '#FEF3C7';
                            const border = it.faturado ? '#A05040' : '#FCD34D';
                            const textColor = it.faturado
                              ? '#F5D5CC'
                              : '#92400E';
...
// ─── RELATÓRIO ────────────────────────────────────────────────────────────────
function RelatorioTab({ perfil = 'Administrador' }) {
          ...
          itens.length === 0
            ...
            : itens
                .map((it) => {
                  ...
                  const statusFat = it.faturado
                    ? `FAT. em ${fmtDate(it.dataFaturamento)}${
                        it.prevEntrega
                          ? ' | Prev. Entrega: ' + fmtDate(it.prevEntrega)
                          : ''
                      }`
                    : it.emAtraso
                    ...
                    ? 'EM ATRASO'
                    : it.prazoPrev
                    ? `Prev. Fat. ${fmtDate(it.prazoPrev)}`
                    : 'pendente';
                  return `${it.codigo} x${it.quantidade} [${statusFat}]`;
                })
                .join(' | ');
        return {
          'Nº Pedido': p.numero,
  ...
  function itensHtml(itensJson, pendencia) {
    ...
    const rows = itens
      .map((it) => {
        ...
        const label = it.faturado
          ? `Fat. ${fmtDate(it.dataFaturamento)}${
              it.prevEntrega ? ' · Entrega: ' + fmtDate(it.prevEntrega) : ''
            }`
          : it.emAtraso
          ? '🔴 Conf. nova data'
          : vencido
          ? '🔴 Em atraso'
          : `⏳ Prev. Fat. ${it.prazoPrev ? fmtDate(it.prazoPrev) : 's/prazo'}`;
...
<table>
  <tr><td colspan="${
    selCols.length
  }" class="header">VÉRTICE — Relatório de ${tipoLabel}</td></tr>
  <tr><td colspan="${
    selCols.length
  }" class="subheader">Gestão de Suprimentos · Zettatecck Projetos · Gerado em ${dataHoje}</td></tr>
  ...
  <tr class="total-row"><td colspan="${selCols.length}">Total de ${
      data.length
    } registro(s) · VÉRTICE · ZETTATECCK PROJETOS · ${dataHoje}</td></tr>
</table></body></html>`;
    const blob = new Blob([html], {
      type: 'application/vnd.ms-excel;charset=utf-8',
    });
  ...
  function exportPDF() {
    ...
    <style>
      *{box-sizing:border-box;margin:0;padding:0;}
      ...
      .header{background:linear-gradient(135deg,#0A0F1E,#0F3460);color:#fff;padding:16px 28px;display:flex;align-items:center;justify-content:space-between;}
      ...
      .meta-bar{background:#C07060;color:#fff;padding:8px 28px;font-size:9px;font-weight:700;display:flex;justify-content:space-between;}
      ...
      .footer{background:#F8FAFC;border-top:2px solid #C07060;padding:10px 28px;display:flex;justify-content:space-between;font-size:8px;color:#64748B;margin-top:20px;}
      @media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}
    </style></head><body>
    <div class="header">
      <div style="display:flex;align-items:center;gap:14px;">
        <div style="display:flex;align-items:center;gap:16px;">
          <svg width="44" height="44" viewBox="-55 -55 110 110">
            <polygon points="0,-48 42,24 -42,24" fill="none" stroke="#4A7FA0" stroke-width="2.5"/>
            <polygon points="0,-33 28,16 -28,16" fill="none" stroke="#4A7FA0" stroke-width="1" opacity="0.4"/>
            <circle cx="0" cy="-33" r="6" fill="#C07060"/>
            <circle cx="28" cy="16" r="3" fill="#C07060" opacity="0.5"/>
            <circle cx="-28" cy="16" r="3" fill="#C07060" opacity="0.5"/>
            <line x1="0" y1="-33" x2="0" y2="16" stroke="#C07060" stroke-width="2" opacity="0.7"/>
            <line x1="0" y1="-33" x2="28" y2="16" stroke="#C07060" stroke-width="1" opacity="0.3"/>
            <line x1="0" y1="-33" x2="-28" y2="16" stroke="#C07060" stroke-width="1" opacity="0.3"/>
          </svg>
          <div>
            <div style="font-size:22px;font-weight:800;letter-spacing:-0.5px;line-height:1;color:#E8F4FC;">VÉRTICE</div>
            <div style="height:2px;background:#C07060;border-radius:1px;margin-top:3px;"></div>
            <div style="font-size:8px;color:rgba(255,255,255,.4);letter-spacing:2px;text-transform:uppercase;margin-top:3px;">GESTÃO DE SUPRIMENTOS</div>
          </div>
        </div>
        <div style="width:1px;height:32px;background:rgba(255,255,255,.12);margin:0 8px;"></div>
        <div style="display:flex;align-items:center;gap:6px;opacity:0.5;">
          <img src="${LOGO}" style="width:18px;height:18px;border-radius:4px;object-fit:cover;" alt="Zettatecck"/>
          <span style="color:rgba(255,255,255,.9);font-size:9px;letter-spacing:0.5px;">ZETTATECCK PROJETOS</span>
        </div>
      </div>
    ...
    <div class="footer">
      <span><strong>VÉRTICE</strong> — Gestão de Suprimentos · ZETTATECCK PROJETOS</span>
      ...
      {step === 1 && (
        ...
        >
          ...
          {!isAdmin && perfil !== 'Compras' && (
            ...
            >
              👁️ Perfil <strong>{perfil}</strong> — disponível apenas: Pedidos
              de Compra
            </div>
          )}
          <div
...
function AnaliseProjetoTab() {
  ...
  const { data: empilhadeiras, loading: lEm } = useCRUD(
    'empilhadeiras',
    'order=criado_em.desc'
  );
  const [search, setSearch] = useState('');
  ...
  if (lP || lPed || lFr || lEm) return <Spin />;
  ...
  const projetosComCalculo = projetos.map((proj) => {
    ...
    const fretesDoProjeto = fretes.filter(
      (f) => String(f.projeto) === String(proj.numero)
    );
    const empilhadeirasDoProjeto = empilhadeiras.filter(
      (e) => String(e.projeto) === String(proj.numero)
    );
    const totalPedidos = pedidosDoProjeto.reduce(
      (s, p) => s + (p.valor || 0),
      0
    );
    const totalFretePedidos = pedidosDoProjeto.reduce(
      (s, p) => s + (p.valor_frete || 0),
      0
    );
    const totalFretesAba = fretesDoProjeto.reduce(
      (s, f) => s + (f.valor || 0),
      0
    );
    const totalEmpilhadeiras = empilhadeirasDoProjeto.reduce(
      (s, e) => s + (e.valor || 0),
      0
    );
    const totalFrete = totalFretePedidos + totalFretesAba;
    const totalComFrete = totalPedidos + totalFrete + totalEmpilhadeiras;
    ...
    const saldo = (proj.max_compras || 0) - totalComFrete;
    const pctUsado =
      proj.max_compras > 0
        ? Math.round((totalComFrete / proj.max_compras) * 100)
        : 0;
    const isNestle = ['NESTLÉ', 'NESTLE', 'GAROTO', 'CPW', 'PURINA'].some((x) =>
      (proj.cliente || '').toUpperCase().includes(x)
    );
    const pctRegra = isNestle ? 35 : 40;
    return {
      ...proj,
      pedidos: pedidosDoProjeto,
      fretes: fretesDoProjeto,
      empilhadeiras: empilhadeirasDoProjeto,
      totalPedidos,
      totalFrete,
      totalFretesAba,
      totalEmpilhadeiras,
            ...
            >
              ...
              <span
                style={{
                  fontSize: 11,
                  fontWeight: 600,
                  background: p.pctRegra === 35 ? '#DCFCE7' : '#DBEAFE',
                  color: p.pctRegra === 35 ? '#166534' : '#1E40AF',
                  padding: '2px 8px',
                  borderRadius: 10,
                  flexShrink: 0,
                }}
              >
                {p.pctRegra === 35 ? 'Nestlé 35%' : 'Cliente 40%'}
              </span>
            </div>
                  ...
                  style={{
                    ...
                    background:
                      projSel.pctRegra === 35
                        ? 'rgba(16,185,129,.2)'
                        : 'rgba(59,130,246,.2)',
                    color: projSel.pctRegra === 35 ? '#34D399' : '#60A5FA',
                    padding: '3px 10px',
                    borderRadius: 20,
                  }}
                >
                  {projSel.pctRegra === 35 ? 'Nestlé — 35%' : 'Cliente — 40%'}
                </span>
              </div>
            </div>
            {/* 4 KPIs */}
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {[
                ...
                ['Total em pedidos', fmt(projSel.totalPedidos), '#3B82F6'],
                ...(projSel.totalFrete > 0
                  ? [['Total em fretes', fmt(projSel.totalFrete), '#F97316']]
                  : []),
                ...(projSel.totalEmpilhadeiras > 0
                  ? [
                      [
                        'Total em empilhadeiras',
                        fmt(projSel.totalEmpilhadeiras),
                        '#8B5CF6',
                      ],
                    ]
                  : []),
                [
                  'Total geral (c/ frete e empilhadeira)',
                  fmt(projSel.totalComFrete),
                  '#0F172A',
                ],
                [
            ...
            {/* Empilhadeiras do projeto */}
            {projSel.empilhadeiras?.length > 0 && (
              <div style={{ marginTop: 10 }}>
                ...
                >
                  🏗️ Empilhadeiras do Projeto
                </div>
                {projSel.empilhadeiras.map((e) => (
                  <div
                    key={e.id}
                    style={{
                      display: 'flex',
...
function FretesTab() {
  const {
    ...
    atualizar,
    deletar,
  } = useCRUD('fretes', 'order=criado_em.desc');
  const { data: fornecedores } = useCRUD('fornecedores');
  const { data: projetos } = useCRUD('projetos');
  const [modal, setModal] = useState(null);
  const [form, setForm] = useState({});
  ...
  function openNew() {
    const nextNum = fretes.length + 1;
    setForm({
      id: '',
      numero: `FRETE ${String(nextNum).padStart(2, '0')}`,
      tipo: 'Remetente',
      fornecedor: '',
      projeto: '',
      valor: '',
  ...
  async function del(id) {
    if (window.confirm('Excluir este frete?')) await deletar(id);
  }
  if (loading) return <Spin />;
  const filt = fretes.filter(
    (f) =>
      !search ||
      f.numero?.toLowerCase().includes(search.toLowerCase()) ||
      f.fornecedor?.toLowerCase().includes(search.toLowerCase()) ||
      (f.projeto || '').toLowerCase().includes(search.toLowerCase())
  );
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div
      ...
      >
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="🔍 Buscar frete, transportadora, projeto..."
          style={{ ...INP, flex: 1, minWidth: 180 }}
        />
        {search && (
          <button
        ...
        <button onClick={openNew} style={{ ...SAVE_BTN, padding: '9px 20px' }}>
          + Novo Frete
        </button>
      </div>
      <div
      ...
      >
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filt.map((f, i) => (
              <tr key={f.id} className="tbl-row" style={{ background: '#fff' }}>
                <TD>
                  <div style={{ display: 'flex', gap: 5 }}>
                    <Btn
                      icon="✏️"
                ...
                <TD style={{ fontWeight: 600 }}>{f.fornecedor}</TD>
                ...
                <TD style={{ color: '#64748B' }}>{fmtDate(f.data_coleta)}</TD>
                <TD style={{ color: '#1E3A5F', fontWeight: 700 }}>
                  {f.horario_coleta || '—'}
                </TD>
                <TD style={{ color: '#64748B' }}>{fmtDate(f.data_entrega)}</TD>
      ...
      {modal && (
        <Modal
          title={modal === 'new' ? 'Novo Frete' : 'Editar Frete'}
          onClose={() => setModal(null)}
        >
          <div
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}
          >
            <div>
              <label style={LBL}>Nº do Frete</label>
              <input
                style={INP}
                value={form.numero || ''}
                onChange={(e) => set('numero', e.target.value)}
            ...
            <div>
              ...
              <select
                style={INP}
                value={form.tipo || 'Remetente'}
                onChange={(e) => set('tipo', e.target.value)}
              >
                <option value="Remetente">📤 Por conta do Remetente</option>
            ...
            <div>
              <label style={LBL}>CT-e</label>
              <input
                style={INP}
                value={form.cte || ''}
                onChange={(e) => set('cte', e.target.value)}
                placeholder="Nº do conhecimento (opcional)"
              />
            </div>
            <div>
              <label style={LBL}>Data de Coleta</label>
              <input
                style={INP}
                type="date"
            ...
            <div>
              <label style={{ ...LBL, color: '#1E3A5F' }}>
                ⏰ Horário de Coleta
              </label>
              <input
                style={{
                  ...INP,
                  borderColor: form.horario_coleta ? '#1E3A5F' : '#E8EDF4',
                }}
                type="time"
                value={form.horario_coleta || ''}
                onChange={(e) => set('horario_coleta', e.target.value)}
              />
            </div>
            <div style={{ gridColumn: 'span 2' }}>
          ...
          >
            ...
            <button
              onClick={save}
              disabled={saving || !form.fornecedor || !form.projeto}
              style={{
                ...SAVE_BTN,
                opacity: saving || !form.fornecedor || !form.projeto ? 0.6 : 1,
              }}
            >
              {saving ? 'Salvando...' : '💾 Salvar'}
            </button>
...
function EmpilhadeirasTab() {
  const {
    ...
    atualizar,
    deletar,
  } = useCRUD('empilhadeiras', 'order=criado_em.desc');
  const { data: fornecedores } = useCRUD('fornecedores');
  const { data: projetos } = useCRUD('projetos');
  const [modal, setModal] = useState(null);
  const [form, setForm] = useState({});
  ...
  function openNew() {
    const nextNum = empilhadeiras.length + 1;
    setForm({
      id: '',
      numero: `EMPILHADEIRA ${String(nextNum).padStart(2, '0')}`,
      tipo: 'Remetente',
      fornecedor: '',
      projeto: '',
      valor: '',
  ...
  async function del(id) {
    if (window.confirm('Excluir esta empilhadeira?')) await deletar(id);
  }
  if (loading) return <Spin />;
  const filt = empilhadeiras.filter(
    (f) =>
      !search ||
      f.numero?.toLowerCase().includes(search.toLowerCase()) ||
      f.fornecedor?.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      >
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          placeholder="🔍 Buscar empilhadeira, locadora, projeto..."
          style={{ ...INP, flex: 1, minWidth: 180 }}
        />
        {search && (
          <button
      ...
      >
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {filt.map((e, i) => (
              <tr key={e.id} className="tbl-row" style={{ background: '#fff' }}>
                <TD>
                  <div style={{ display: 'flex', gap: 5 }}>
                    <Btn
                      icon="✏️"
                ...
                <TD>
                  <span
                    style={{
                      ...
                      borderRadius: 20,
                      background:
                        e.tipo === 'Remetente' ? '#EFF6FF' : '#FFF7ED',
                      color: e.tipo === 'Remetente' ? '#1E40AF' : '#C2410C',
                    }}
                  >
                    {e.tipo === 'Remetente'
                      ? '📤 Remetente'
                ...
                <TD style={{ fontWeight: 600 }}>{e.fornecedor}</TD>
                <TD style={{ color: '#3B82F6', fontWeight: 600 }}>
                  {e.projeto}
                </TD>
                <TD style={{ fontWeight: 700, color: '#1E3A5F' }}>
                ...
                <TD style={{ color: '#64748B' }}>{fmtDate(e.data_reserva)}</TD>
              </tr>
            ))}
            {filt.length === 0 && (
              <tr>
      ...
      {modal && (
        <Modal
          title={modal === 'new' ? 'Nova Empilhadeira' : 'Editar Empilhadeira'}
          onClose={() => setModal(null)}
        >
          <div
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}
          >
            <div>
              <label style={LBL}>Nº da Empilhadeira</label>
              <input
                style={INP}
                value={form.numero || ''}
                onChange={(e) => set('numero', e.target.value)}
            ...
            <div>
              <label style={LBL}>Locadora</label>
              <select
                style={INP}
                value={form.fornecedor || ''}
                onChange={(e) => set('fornecedor', e.target.value)}
            ...
            <div>
              <label style={LBL}>Valor da Locação (R$)</label>
              <input
                style={INP}
                type="number"
                value={form.valor || ''}
            ...
            <div>
              <label style={LBL}>Data de Reserva</label>
              <input
                style={INP}
                type="date"
                value={form.data_reserva || ''}
                onChange={(e) => set('data_reserva', e.target.value)}
              />
            </div>
          </div>
          <div
...
const PRIO_COLORS = {
  Alta: { bg: '#FFF1F2', text: '#E11D48', border: '#FECDD3', dot: '#F43F5E' },
  Normal: { bg: '#EFF6FF', text: '#2563EB', border: '#BFDBFE', dot: '#3B82F6' },
  Baixa: { bg: '#F0FDF4', text: '#059669', border: '#BBF7D0', dot: '#10B981' },
};

function LembretesTab({ usuario, perfil }) {
  ...
  const [saving, setSaving] = useState(false);
  const [filtro, setFiltro] = useState('todos'); // todos | meus | compartilhados

  const carregar = useCallback(async () => {
    setLoading(true);
    try {
      const d = await sbGet('lembretes', 'order=criado_em.desc');
      setLembretes(d);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const usuarioId = usuario.id || usuario.email;

  function podeVer(l) {
    if (l.usuario_id === usuarioId) return true;
    if (l.visibilidade === 'Todos') return true;
    if (l.visibilidade === 'Grupos') {
      const grupos = JSON.parse(l.grupos_acesso || '[]');
      return grupos.includes(perfil);
    }
    return false;
  }

  const lembFiltrados = lembretes.filter((l) => {
    if (!podeVer(l)) return false;
    if (filtro === 'meus') return l.usuario_id === usuarioId;
    if (filtro === 'compartilhados') return l.usuario_id !== usuarioId;
    return true;
  });
  ...
  async function salvar() {
    ...
    try {
      ...
      const obj = {
        ...
        grupos_acesso:
          form.visibilidade === 'Grupos' ? JSON.stringify(form.grupos) : '[]',
        data_vencimento: form.sem_prazo ? null : form.data_vencimento || null,
      };
      await sbInsert('lembretes', obj);
      setModal(false);
      setForm({
        titulo: '',
        descricao: '',
  ...
  async function excluir(id) {
    ...
    await fetch(`${SUPA_URL}/rest/v1/lembretes?id=eq.${id}`, {
      method: 'DELETE',
      headers: H,
    });
    carregar();
  ...
  function toggleGrupo(g) {
    setForm((f) => ({
      ...f,
      grupos: f.grupos.includes(g)
        ? f.grupos.filter((x) => x !== g)
        : [...f.grupos, g],
    }));
  }
  ...
  function statusPrazo(l) {
    ...
    const prazo = parseLocal(l.data_vencimento);
    const diff = Math.ceil((prazo - hoje) / (1000 * 60 * 60 * 24));
    if (diff < 0)
      return {
        label: `Vencido há ${Math.abs(diff)} dia${
          Math.abs(diff) > 1 ? 's' : ''
        }`,
        color: '#E11D48',
        bg: '#FFF1F2',
    ...
    if (diff === 0)
      return { label: 'Vence hoje!', color: '#D97706', bg: '#FFFBEB' };
    if (diff <= 3)
      return {
    ...
    return {
      label: `Prazo: ${fmtDate(l.data_vencimento)}`,
      color: '#059669',
      bg: '#ECFDF5',
    };
  }
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                border: 'none',
                borderRadius: 6,
                background: filtro === v ? '#fff' : 'transparent',
                color: filtro === v ? '#0D1B2A' : '#64748B',
                fontWeight: filtro === v ? 700 : 500,
                fontSize: 12,
                cursor: 'pointer',
                fontFamily: FONT,
                boxShadow: filtro === v ? '0 1px 4px rgba(0,0,0,.1)' : 'none',
      ...
      {loading ? (
        ...
        >
          ...
          <div style={{ color: '#94A3B8', fontSize: 13 }}>
            Crie um novo lembrete clicando no botão acima
          </div>
        </div>
      ) : (
        <div
        ...
        >
          {lembFiltrados.map((l) => {
            const prio = PRIO_COLORS[l.prioridade] || PRIO_COLORS.Normal;
            const prazo = statusPrazo(l);
            const eMeu = l.usuario_id === usuarioId;
            return (
              <div
                key={l.id}
                style={{
      ...
      {modal && (
        <Modal title="🔔 Novo Lembrete" onClose={() => setModal(false)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {/* Grupos */}
            {form.visibilidade === 'Grupos' && (
              <div>
                <label style={LBL}>Quais grupos podem ver?</label>
                <div
                  style={{
                    display: 'flex',
                    gap: 8,
                ...
                >
                  {PERFIS_TODOS.filter(
                    (p) => p !== 'Administrador' || perfil === 'Administrador'
                  ).map((g) => (
                    <button
                      key={g}
                      type="button"
                      ...
                      style={{
                        padding: '6px 14px',
                        border: `2px solid ${
                          form.grupos.includes(g) ? '#1E3A5F' : '#E2E8F0'
                        }`,
                        borderRadius: 20,
                        background: form.grupos.includes(g)
                          ? '#EFF6FF'
                          : '#fff',
                        color: form.grupos.includes(g) ? '#1E3A5F' : '#64748B',
                        fontWeight: form.grupos.includes(g) ? 700 : 500,
                        fontSize: 12,
                        cursor: 'pointer',
                        fontFamily: FONT,
                        transition: 'all 0.15s',
            ...
            {/* Prazo */}
            <div>
              <label style={LBL}>Prazo / Vencimento</label>
              <div
                style={{
                  display: 'flex',
                  alignItems: 'center',
            ...
            >
              <button onClick={() => setModal(false)} style={CANCEL_BTN}>
                Cancelar
              </button>
              <button
                onClick={salvar}
                disabled={
                  saving ||
                  !form.titulo.trim() ||
                  (form.visibilidade === 'Grupos' && form.grupos.length === 0)
                }
                style={{
                  ...SAVE_BTN,
                  opacity:
...
export default function App() {
  ...
  // normaliza perfil — case-insensitive para evitar problemas de capitalização
  const perfilRaw = usuario?.perfil || '';
  const perfil = (() => {
    const p = perfilRaw.trim().toLowerCase();
    if (p === 'administrador') return 'Administrador';
    if (p === 'compras') return 'Compras';
    if (p === 'engenharia') return 'Engenharia';
    if (p === 'comercial') return 'Comercial';
    if (p === 'almoxarifado') return 'Almoxarifado';
    return 'Compras'; // fallback
  })();
  const isAdmin = perfil === 'Administrador';
  const isReadOnly = ['Engenharia', 'Comercial', 'Almoxarifado'].includes(
    perfil
  );
  ...
  const TABS_ALL = [
    ...
    { id: 'fretes', label: '🚚 Fretes', perfis: ['Administrador', 'Compras'] },
    {
      id: 'empilhadeiras',
      label: '🏗️ Empilhadeiras',
      perfis: ['Administrador', 'Compras'],
    },
    {
      id: 'analise',
      label: '📋 Análise por Projeto',
      perfis: ['Administrador', 'Compras', 'Engenharia', 'Comercial'],
    },
    {
      id: 'relatorio',
      label: '📤 Relatório',
  ...
  const TABS = TABS_ALL.filter(
    (t) => Array.isArray(t.perfis) && t.perfis.includes(perfil)
  );
    ...
    >
      ...
      <div
        style={{
          ...
          color: '#fff',
          width: '100%',
          boxShadow: '0 4px 20px rgba(13,27,42,.4)',
        }}
      >
        <div
          style={{ width: '100%', padding: '0 24px', boxSizing: 'border-box' }}
        >
          <div
            style={{
              display: 'flex',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                  {/* Triângulo VÉRTICE */}
                  <svg width="52" height="52" viewBox="-55 -55 110 110">
                    <polygon
                      points="0,-48 42,24 -42,24"
                      fill="none"
                      stroke="#4A7FA0"
          ...
          >
            {TABS.map((t) => (
              <button
                ...
                style={{
                  padding: '9px 16px',
                  background: tabAtual === t.id ? '#EEF2F7' : 'transparent',
                  border: 'none',
                  borderRadius: '8px 8px 0 0',
                  color: tabAtual === t.id ? '#0D1B2A' : 'rgba(255,255,255,.5)',
                  fontWeight: tabAtual === t.id ? 700 : 400,
                  fontSize: 12,
                  cursor: 'pointer',
                  fontFamily: FONT,
                  whiteSpace: 'nowrap',
      ...
      <div
        style={{ width: '100%', padding: '20px 24px', boxSizing: 'border-box' }}
      >
        {isReadOnly && (
          <div
            style={{
          ...
          >
            👁️ Modo visualização — perfil <strong>{perfil}</strong> · Sem
            permissão para editar ou excluir dados
          </div>
        )}
        {tabAtual === 'dashboard' && <Dashboard />}
        {tabAtual === 'projetos' && <ProjetosTab />}
        {tabAtual === 'pedidos' && <PedidosTab />}
        {tabAtual === 'pagamentos' && <PagamentosTab />}
        {tabAtual === 'fornecedores' && <FornecedoresTab />}
        {tabAtual === 'fretes' && <FretesTab />}
        {tabAtual === 'empilhadeiras' && <EmpilhadeirasTab />}
        {tabAtual === 'analise' && <AnaliseProjetoTab />}
        {tabAtual === 'relatorio' && <RelatorioTab perfil={perfil} />}
        {tabAtual === 'lembretes' && (
          <LembretesTab usuario={usuario} perfil={perfil} />
        )}
        {tabAtual === 'usuarios' && perfil === 'Administrador' && (
          <UsuariosTab />
        )}
      </div>
    </div>
